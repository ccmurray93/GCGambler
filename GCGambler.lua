-- Version 4.30
-- (Myxini-Hyjal) Updated for patch 5.4 change to Player-Realm name format
-- (Myxini-Hyjal) Ignores players from other realms
...
function GCGambler_OnLoad(this)
	DEFAULT_CHAT_FRAME:AddMessage("|cffffff00<Gentlemen's Club Gambler v4.30> loaded /gcg to use");
	...
	hostRealm = strlower(GetRealmName())
...
function GCGambler_OnEvent(self, event, ...)
	...
	if ((event == "CHAT_MSG_RAID_LEADER" or event == "CHAT_MSG_RAID") and AcceptOnes=="true" and GCGambler["chat"] == true) then
		-- Splits arg2 into player name and realm
		local player, realm = strsplit("-", strlower(arg2))

		-- ADDS USER TO THE ROLL POOL - CHECK TO MAKE SURE THEY ARE NOT BANNED --
		if (arg1 == "1") then
			if (hostRealm == realm) then
				if(GCGambler_ChkBan(tostring(player)) == 0) then
					GCGambler_Add(tostring(player));
				...
				SendChatMessage("Unable to accept entry. You must be on the same realm as the host.", chatmethod);
			end
		elseif(arg1 == "-1") then
